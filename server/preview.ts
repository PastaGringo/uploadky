...
/**
 * The origin as the CLIENT reached it, for og:url and canonical.
 *
 * Header-derived, so it must never feed a fetch or any server-side decision —
 * a forged `x-forwarded-host` would redirect that request. The Bun server sits
 * behind a proxy that rewrites this header from the real Host; the Vercel
 * function uses `trustedOrigin()` instead, for exactly this reason.
 */
...
/**
 * The origin this function trusts for its OWN URLs.
 *
 * NEVER derived from request headers. `publicOrigin()` reads
 * `x-forwarded-host`, which a client can set: it would send the fetch below to
 * a third party and serve their HTML from this origin — the origin that holds
 * the session snapshot and the homeserver cookie. The response carries
 * `public, max-age=60`, so a poisoned copy could be cached and handed to
 * everyone else.
 *
 * Measured: a forged `Host` is rejected by the platform, a forged
 * `X-Forwarded-Host` is not.
 *
 * Vercel sets both variables itself and a request cannot influence them.
 * Outside Vercel, the URL the runtime received is the fallback — still not a
 * header.
 */
export function trustedOrigin(request: Request): string {
  const host =
    process.env.UPLOADKY_PUBLIC_ORIGIN?.trim() ||
